...
describe("update-check", () => {
  ...
  describe("classifyInstallPath", () => {
    it("returns 'npm-global' for /usr/local/lib/node_modules path", () => {
      expect(
        classifyInstallPath(
          "/usr/local/lib/node_modules/@slievr/cli/dist/lib/update-check.js",
        ),
      ).toBe("npm-global");
    });
    ...
    it("returns 'npm-global' for nvm global path", () => {
      expect(
        classifyInstallPath(
          "/home/user/.nvm/versions/node/v20.0.0/lib/node_modules/@slievr/cli/dist/lib/update-check.js",
        ),
      ).toBe("npm-global");
    });
    ...
    it("returns 'pnpm-global' for pnpm global store path", () => {
      expect(
        classifyInstallPath(
          "/home/user/.local/share/pnpm/global/5/node_modules/.pnpm/@aoagents+ao-cli@0.2.2/node_modules/@slievr/cli/dist/lib/update-check.js",
        ),
      ).toBe("pnpm-global");
    });
    ...
    it("returns 'unknown' for local pnpm node_modules/.pnpm (not global)", () => {
      ...
      expect(
        classifyInstallPath(
          "/home/user/my-project/node_modules/.pnpm/@aoagents+ao-cli@0.2.2/node_modules/@slievr/cli/dist/lib/update-check.js",
        ),
      ).toBe("unknown");
    });
    ...
    it("returns 'unknown' for local project node_modules (npx)", () => {
      ...
      expect(
        classifyInstallPath(
          "/home/user/my-project/node_modules/@slievr/cli/dist/lib/update-check.js",
        ),
      ).toBe("unknown");
    });
    ...
    it("returns 'git' when repo root has .git", () => {
      ...
      mockReadFileSync.mockReturnValue(JSON.stringify({ name: "@slievr/athene" }));
  ...
  describe("getUpdateCommand", () => {
    ...
    it("returns npm install command for npm-global installs", () => {
      expect(getUpdateCommand("npm-global")).toBe("npm install -g @slievr/athene@latest");
    });

    it("returns pnpm add command for pnpm-global installs", () => {
      expect(getUpdateCommand("pnpm-global")).toBe("pnpm add -g @slievr/athene@latest");
    });

    it("returns npm install command for unknown installs", () => {
      expect(getUpdateCommand("unknown")).toBe("npm install -g @slievr/athene@latest");
    });
  });
  ...
  describe("fetchLatestVersion", () => {
    it("returns latest dist-tag from registry by default (stable channel)", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "https://registry.npmjs.org/@slievr%2Fathene",
  ...
  describe("checkForUpdate", () => {
    ...
    it("uses npm registry and npm-global command for npm-global installs", async () => {
      ...
      expect(info.latestVersion).toBe("0.3.0");
      expect(info.recommendedCommand).toBe("npm install -g @slievr/athene@latest");
    });
    ...
    it("uses npm registry and pnpm-global command for pnpm-global installs", async () => {
      ...
      expect(info.latestVersion).toBe("0.3.0");
      expect(info.recommendedCommand).toBe("pnpm add -g @slievr/athene@latest");
    });
    ...
    it("uses cached git state without consulting npm registry", async () => {
      ...
      mockReadFileSync.mockImplementation((path: string) => {
        ...
        return JSON.stringify({ name: "@slievr/athene" });
      });
  ...
  describe("maybeShowUpdateNotice", () => {
    ...
    it("prints update notice when cache shows outdated version", () => {
      ...
      expect(output).toContain("npm install -g @slievr/athene@latest");
    });
    ...
    it("does not print placeholder 0.0.0 when the current version is unknown", () => {
      ...
      expect(output).toContain("npm install -g @slievr/athene@nightly");
  ...
  describe("classifyInstallPath — bun + homebrew (Section F)", () => {
    it("returns 'bun-global' for ~/.bun/install/global/ paths", () => {
      expect(
        classifyInstallPath(
          "/home/user/.bun/install/global/node_modules/@slievr/cli/dist/lib/update-check.js",
        ),
      ).toBe("bun-global");
    });
    ...
    it("returns 'homebrew' for /Cellar/ao/ paths even when nested under lib/node_modules", () => {
      ...
      expect(
        classifyInstallPath(
          "/usr/local/Cellar/ao/0.5.0/libexec/lib/node_modules/@slievr/cli/dist/lib/update-check.js",
        ),
      ).toBe("homebrew");
    });
  });
  ...
  describe("getUpdateCommand — channel-aware (Section B)", () => {
    it("uses @nightly tag for nightly channel", () => {
      expect(getUpdateCommand("npm-global", "nightly")).toBe("npm install -g @slievr/athene@nightly");
      expect(getUpdateCommand("pnpm-global", "nightly")).toBe("pnpm add -g @slievr/athene@nightly");
      expect(getUpdateCommand("bun-global", "nightly")).toBe("bun add -g @slievr/athene@nightly");
    });

    it("uses @latest tag for stable + manual channels", () => {
      expect(getUpdateCommand("npm-global", "stable")).toBe("npm install -g @slievr/athene@latest");
      expect(getUpdateCommand("npm-global", "manual")).toBe("npm install -g @slievr/athene@latest");
    });
