 ...
 * Locates @slievr/core by walking up from webDir, handling both pnpm
...
async function checkBuilt(webDir: string): Promise<void> {
  ...
  const corePkgDir = findPackageUp(webDir, "@slievr", "core");
  if (!corePkgDir) {
    const hint = isNpmInstall
      ? "Run: npm install -g @slievr/athene@latest"
