...
describe("generateOrchestratorPrompt", () => {
  ...
  it("mandates athene send and bans raw runtime access", async () => {
    const generateOrchestratorPrompt = await loadGenerateOrchestratorPrompt();
    const prompt = generateOrchestratorPrompt({
      config,
      projectId: "my-app",
    ...
    expect(prompt).toContain("Always use `athene send`");
    ...
    expect(prompt).toContain("athene send --no-wait");
  });
  ...
  it("shows 'not configured' and omits repo-only guidance when repo is omitted", async () => {
    ...
    expect(prompt).not.toContain("athene batch-spawn");
    expect(prompt).not.toContain("athene session claim-pr");
  ...
  it("expands markdown template placeholders with typed render data", async () => {
    ...
    expect(prompt).toContain("athene session ls -p my-app");
