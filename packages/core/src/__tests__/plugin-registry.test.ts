...
describe("loadBuiltins", () => {
  ...
  it("registers multiple agent plugins from importFn", async () => {
    ...
    await registry.loadBuiltins(undefined, async (pkg: string) => {
      if (pkg === "@slievr/plugin-agent-claude-code") return fakeClaudeCode;
      if (pkg === "@slievr/plugin-agent-codex") return fakeCodex;
      if (pkg === "@slievr/plugin-agent-grok") return fakeGrok;
      if (pkg === "@slievr/plugin-agent-opencode") return fakeOpenCode;
      throw new Error(`Not found: ${pkg}`);
    });
  ...
  it("registers gitlab tracker and scm plugins from importFn", async () => {
    ...
    await registry.loadBuiltins(undefined, async (pkg: string) => {
      if (pkg === "@slievr/plugin-tracker-gitlab") return fakeTracker;
      if (pkg === "@slievr/plugin-scm-gitlab") return fakeScm;
      throw new Error(`Not found: ${pkg}`);
    });
  ...
  it("passes configured notifier plugin config to create()", async () => {
    ...
    await registry.loadBuiltins(config, async (pkg: string) => {
      if (pkg === "@slievr/plugin-notifier-webhook") return fakeWebhookNotifier;
      throw new Error(`Not found: ${pkg}`);
    });
  ...
  it("registers alias-specific notifier instances for the same plugin", async () => {
    ...
    await registry.loadBuiltins(config, async (pkg: string) => {
      if (pkg === "@slievr/plugin-notifier-slack") return fakeSlackNotifier;
      throw new Error(`Not found: ${pkg}`);
    });
  ...
  it("passes notifier config from config.notifiers when loading builtins", async () => {
    ...
    await registry.loadBuiltins(cfg, async (pkg: string) => {
      if (pkg === "@slievr/plugin-notifier-openclaw") return fakeOpenClaw;
      throw new Error(`Not found: ${pkg}`);
    });
  ...
  it("passes configPath for named notifier references without explicit config", async () => {
    ...
    await registry.loadBuiltins(cfg, async (pkg: string) => {
      if (pkg === "@slievr/plugin-notifier-dashboard") return fakeDashboard;
      throw new Error(`Not found: ${pkg}`);
    });
  ...
  it("strips package loading metadata from notifier config", async () => {
    ...
    const cfg = makeOrchestratorConfig({
      ...
      notifiers: {
        mywebhook: {
          ...
          package: "@slievr/plugin-notifier-webhook",
    ...
    await registry.loadBuiltins(cfg, async (pkg: string) => {
      if (pkg === "@slievr/plugin-notifier-webhook") return fakeWebhook;
      throw new Error(`Not found: ${pkg}`);
    });
  ...
  it("should use provided importFn instead of built-in import", async () => {
    ...
    const fakeImportFn = async (pkg: string): Promise<unknown> => {
      ...
      if (pkg === "@slievr/plugin-runtime-tmux") {
        return {
    ...
    expect(importedPackages).toContain("@slievr/plugin-runtime-tmux");
