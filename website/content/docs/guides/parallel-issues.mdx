...
`athene batch-spawn` takes one or more issue identifiers:

```bash
athene batch-spawn 42 43 44
```
...
For a quick interactive version, just run `athene spawn` a few times — `batch-spawn` adds the duplicate detection.
...
```bash
athene spawn 42 --agent claude-code
athene spawn 42 --agent codex
```
...
athene status -p my-repo --json
```

## When you want fewer

- `athene session kill <sessionId>` — kill one
- `athene session cleanup` — kill everything whose PR merged or issue closed (safe; archives metadata)
- `athene session cleanup --dry-run` — preview first

<Callout type="info">
  Need them all gone? `athene stop --all` halts every running AO instance on this machine.
</Callout>
