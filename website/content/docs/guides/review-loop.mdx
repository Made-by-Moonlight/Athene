...
```bash
athene review-check           # check all tracked PRs right now
athene review-check myproject # one project only
athene review-check --dry-run # show what would be sent, don't send
```

`athene review-check` is a standalone CLI command that calls the GitHub API directly and is not subject to the in-process throttle.
