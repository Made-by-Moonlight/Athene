...
<Accordions>
  ...
  <Accordion title="PR created but dashboard doesn't know">
    The PATH wrapper writes session metadata on `gh pr create`. If `gh` was invoked with an absolute path (bypassing the wrapper) AO won't see it. `athene session claim-pr <pr> <sessionId>` fixes this retroactively.
  </Accordion>
</Accordions>
