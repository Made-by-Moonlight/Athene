...
<Accordions>
  ...
  <Accordion title="Dashboard never leaves `active`">
    Cursor doesn't emit its own session JSONL. AO classifies based on terminal output — if your terminal is being captured by another tool, the classification can drift. `athene doctor` will flag this.
  </Accordion>
</Accordions>
