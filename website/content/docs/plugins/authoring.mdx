...
```typescript
import type { PluginModule, Notifier } from "@slievr/core";
...
Replace the placeholder `create()` body with a real implementation. Import the slot interface from `@slievr/core`:

```typescript
import type { PluginModule, Notifier } from "@slievr/core";
import { validateUrl } from "@slievr/core";
```
...
All interfaces are defined in `packages/core/src/types.ts` and exported from `@slievr/core`.
...
All utilities are exported from `@slievr/core`. The source lives in `packages/core/src/index.ts`.
