...
The dashboard already has an xterm.js terminal connected to every session. The `web` plugin just points `athene open` at the right dashboard URL.
...
When you run `athene open <session>`, the plugin prints the session's dashboard URL. If you're on macOS/Linux, it also tries to open it in your default browser.
