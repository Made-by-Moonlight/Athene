...
<Tabs items={["GitHub issue", "Freeform prompt"]}>
  ...
  ```bash
  athene spawn --prompt "Update the README install section to mention Node 20"
  ```
...
Use `athene status` when you want the same high-level view in a terminal:
...
If you need to give the worker a direct instruction, use `athene send`:

```bash
athene send myrepo-1 "Keep the fix smaller. Do not refactor the API layer."
```

Use `athene send` instead of raw terminal input. It preserves AO's busy detection, retry handling, and message formatting.
...
```bash
athene session cleanup --dry-run
...
| Orchestrator session | A coordinating session started by `athene start`; it supervises worker sessions. |
...
| Symptom | First check |
| --- | --- |
| Dashboard is not updating | Make sure the `athene start` terminal is still running. |
| `athene spawn` warns that AO is not running | Start AO with `athene start` before spawning. |
| GitHub issue or PR data is missing | Run `gh auth status` and check the `repo` field in `agent-orchestrator.yaml`. |
| Agent started but does nothing | Open the session terminal and send a clear instruction with `athene send`. |
